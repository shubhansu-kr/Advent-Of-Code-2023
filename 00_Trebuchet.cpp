...
class Solution1{
    // --- Part 2 ---
public: 
    int getCalibrationValue(vector<string> &str){
        int val = 0;
...
public: 
    int getCalibrationValue(vector<string> &str){
        ...
        for(auto &it: str){
            ...
            for(auto &c: it){
                if (c <= '9' && c >= '0'){
                    if (first == '!' /* || first == '0' */) first = c;
                    last = c;
                }
            }
            
            int cal = (first - '0') * 10 + (last - '0');
...
int main () {
    ...
    while(cin >> s){inp.emplace_back(s);}

    cout << obj.getCalibrationValue(inp);
