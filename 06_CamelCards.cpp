...
covering half of the horizon. The Elf explains that the rocks are all along the part of Desert Island that 
is directly above Island Island, making it hard to even get there. Normally, they use big machines to move 
the rocks and filter the sand, but the machines have broken down because Desert Island recently stopped receiving 
...
hand's bid with its rank (765 * 1 + 220 * 2 + 28 * 3 + 684 * 4 + 483 * 5). So the total winnings in this example 
are 6440.
...
bool myComp(pair<string, int> c1, pair<string, int> c2){

    unordered_map<char, int> m1, m2;

    for(auto &it: c1.first) m1[it]++;
    for(auto &it: c2.first) m2[it]++;
    ...
    for(auto &it: m1) max1 = max(max1, it.second); 
    for(auto &it: m2) max1 = max(max2, it.second);
    ...
    for(int i = 0; i < 13; ++i) rank[order[i]] = 13 - i;

    for(int i = 0; i < n; ++i){
        if (rank[c1.first[i]] != rank[c2.first[i]]) return rank[c1.first[i]] > rank[c2.first[i]];
    }
...
public: 
    ...
    long getWinning(vector<pair<string, int>> &cards) {
