...
public: 
    long long getMarginalError() {
        ...
        long long time = stoll(t), distance = stoll(d);

        for(int j = 0; j < time; ++j) {
            if (j * (time-j) > distance){return (time-2*j+1);}
        }
